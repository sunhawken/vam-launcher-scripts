...
$ZAJE_DST = "Z:\Zaje"
$SESSION_LOG = "$LOG_DIR\sessions_vd.log"
$BG_FOLDER = "$VAM_DIR\background"
$BG_EXTRA = "$VAM_DIR\zajebanan"
$ROOT = $VAM_DIR.Substring(0,2)
$VD_STREAMER = "C:\Program Files\Virtual Desktop Streamer\VirtualDesktop.Streamer.exe"
...
$PROFILE_FILE = "$LOG_DIR\last_profile_vd.txt"
...
if (Test-Path "$LOG_DIR\auto_launch_vd.txt") {
    $val = (Get-Content "$LOG_DIR\auto_launch_vd.txt" -Raw).Trim()
...
# STEAMVR KILL
# =================================================
function Kill-SteamVR {
    foreach ($proc in @("vrserver","vrcompositor","vrmonitor","steamvr")) {
        try { Get-Process $proc -ErrorAction Stop | Stop-Process -Force } catch {}
    }
    Write-Log "SteamVR processes killed." "Magenta"
}
...
function Start-VaM {
    ...
    Write-Log "  Mode    : Virtual Desktop (VR)" "DarkGray"
    Kill-SteamVR
    Start-Sleep 2
    Add-Content $SESSION_LOG "[$timestamp] START $($script:activeProfile) [#$($script:sessionCount)]"
    Force-Prefs
    Disable-DXGI
    $script:vamProcess = Start-Process $VD_STREAMER -ArgumentList "`"$($p.Exe)`"" -WorkingDirectory $VAM_DIR -PassThru
...
$timer.Add_Tick({
    ...
    if ($proc) {
        ...
        $uptime = Get-Uptime
        $form.Text = "$($script:activeProfile) VD  //  ${memGB}GB RAM  //  $uptime  //  Session #$($script:sessionCount)"
    } else {
        $form.Text = "$($script:activeProfile) VD  //  WAITING..."
    }
    Update-QvaroButton
})
$timer.Start()
...
$btnAutoLaunch.Add_Click({
    ...
    $val | Set-Content "$LOG_DIR\auto_launch_vd.txt"
})
...
function Show-BootSequence {
    Write-Log "========================================" "DarkCyan"
    Write-Log "  NEURAL CORE v3.0 VD  //  BOOT SEQUENCE" "Cyan"
    Write-Log "========================================" "DarkCyan"
    Write-Log "" "White"
