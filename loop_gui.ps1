...
$PROFILE_FILE = "$LOG_DIR\last_profile.txt"
...
function Set-BackgroundImage {
    ...
    $newImg = New-Object System.Drawing.Bitmap([System.Drawing.Image]::FromStream($ms))
    $ms.Dispose()
...
if (Test-Path "$LOG_DIR\auto_launch.txt") {
    $val = (Get-Content "$LOG_DIR\auto_launch.txt" -Raw).Trim()
...
function Start-VaM {
    ...
    Write-Log "  Mode    : Desktop (-vrmode none)" "DarkGray"
    Add-Content $SESSION_LOG "[$timestamp] START $($script:activeProfile) [#$($script:sessionCount)]"
    Force-Prefs
    Disable-DXGI
    $script:vamProcess = Start-Process $p.Exe -ArgumentList "-vrmode none" -WorkingDirectory $VAM_DIR -PassThru
...
$timer.Interval = 2000
$timer.Add_Tick({
    if ($script:autoRestart -and $script:vamProcess -and $script:vamProcess.HasExited) {
        $uptime = Get-Uptime
        $timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
    ...
    if ($proc) {
        ...
        $uptime = Get-Uptime
        $form.Text = "$($script:activeProfile)  //  ${memGB}GB RAM  //  $uptime  //  Session #$($script:sessionCount)"
    } else {
        $form.Text = "$($script:activeProfile)  //  WAITING..."
    }
    Update-QvaroButton
})
$timer.Start()
...
$btnAutoLaunch.Add_Click({
    ...
    $val | Set-Content "$LOG_DIR\auto_launch.txt"
})
...
function Show-BootSequence {
    Write-Log "========================================" "DarkCyan"
    Write-Log "  NEURAL CORE v3.0  //  BOOT SEQUENCE" "Cyan"
    Write-Log "========================================" "DarkCyan"
    Write-Log "" "White"
