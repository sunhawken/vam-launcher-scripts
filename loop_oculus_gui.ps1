...
$ZAJE_DST = "Z:\Zaje"
$SESSION_LOG = "$LOG_DIR\sessions_oculus.log"
...
$PROFILE_FILE = "$LOG_DIR\last_profile_oculus.txt"
...
if (Test-Path "$LOG_DIR\auto_launch_oculus.txt") {
    $val = (Get-Content "$LOG_DIR\auto_launch_oculus.txt" -Raw).Trim()
...
function Force-Prefs {
    ...
    Write-Log "Writing optimized Oculus VR prefs..." "Cyan"
...
function Start-VaM {
    ...
    Write-Log "  Mode    : Oculus VR (-vrmode oculus)" "DarkGray"
    Add-Content $SESSION_LOG "[$timestamp] START $($script:activeProfile) [#$($script:sessionCount)]"
    Force-Prefs
    Disable-DXGI
    $script:vamProcess = Start-Process $p.Exe -ArgumentList "-vrmode oculus" -WorkingDirectory $VAM_DIR -PassThru
    $procId = $script:vamProcess.Id
    Write-Log "  PID     : $procId" "Green"
    Start-ISLC
...
$timer.Add_Tick({
    ...
    if ($proc) {
        ...
        $uptime = Get-Uptime
        $form.Text = "$($script:activeProfile) Oculus  //  ${memGB}GB RAM  //  $uptime  //  Session #$($script:sessionCount)"
    } else {
        $form.Text = "$($script:activeProfile) Oculus  //  WAITING..."
    }
    Update-QvaroButton
})
$timer.Start()
...
$btnAutoLaunch.Add_Click({
    ...
    $val | Set-Content "$LOG_DIR\auto_launch_oculus.txt"
})
...
$btnNextImg.Add_Click({ Set-BackgroundImage })
...
$btnMono.Add_Click({
    ...
    if ($state -eq "3x Heap") {
        ...
        if (!(Test-Path $MONO_PATCHED)) {
            ...
            if ($exCRC -ne $MONO_CRC_PATCHED) { Write-Log "Mono 3x CRC mismatch! Expected $MONO_CRC_PATCHED got $exCRC" "Red"; return }
...
$btnFsr.Add_Click({
    ...
    if ($state -eq "ON") {
        foreach ($dir in $PLUGIN_DIRS) {
            ...
            if (Test-Path $def) { Copy-Item $def "$dir\openvr_api.dll" -Force }
            if (Test-Path "$dir\openvr_mod.cfg") { Remove-Item "$dir\openvr_mod.cfg" -Force -Confirm:$false }
        }
...
function Show-BootSequence {
    Write-Log "========================================" "DarkCyan"
    Write-Log "  NEURAL CORE v3.0 Oculus  //  BOOT SEQUENCE" "Cyan"
    Write-Log "========================================" "DarkCyan"
    Write-Log "" "White"
