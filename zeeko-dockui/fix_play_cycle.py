"""
Patch PLAY button on all Zeeko .var files to cycle play/stop:
  startActions: Play Current Clip (leader only)
  endActions:   Stop (leader only)
  transitionActions: []
Replaces the previous Sync_GIRL + Play_GUY pattern entirely.
"""
...
def repack(files, var_out):
    ...
    try:
        ...
        tmp = os.path.join(tempfile.gettempdir(), '_play_cycle.zip')
...
for var_in in targets:
    ...
    try:
        ...
        # Identify leader atoms from UIButton Play Anim N targets
        vt_atoms = set()
        ...
        if not leaders:
            raise RuntimeError(f'no leaders found, vt_atoms={vt_atoms}')
        ...
        for i in range(cnt):
            ...
            new_entry = {
                ...
                "startActions": [make_action(f"Play_{l}", l, "Play Current Clip")
                                 for l in sorted(leaders)],
                "transitionActions": [],
                "endActions":   [make_action(f"Stop_{l}", l, "Stop")
                                 for l in sorted(leaders)],
            }
            dk[ak] = new_entry
        ...
        for i in range(cnt2):
            ...
            print(f"  {os.path.basename(var_out)}: start={sa} end={ea}")
